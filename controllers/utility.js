...
// Handle building the view for updating a utility. 
// query provides the id 
exports.utility_update_Page =  async function(req, res) { 
    console.log("update view for item "+req.query.id) 
    try{ 
        let result = await Utility.findById(req.query.id) 
        res.render('utilityupdate', { title: 'Utility Update', toShow: result }); 
    } 
    catch(err){ 
        res.status(500) 
        res.send(`{'error': '${err}'}`); 
...
exports.utility_delete_Page = async function(req, res) { 
    console.log("Delete view for id "  + req.query.id) 
    try{ 
        result = await Utility.findById(req.query.id) 
        res.render('utilitydelete', { title: 'Utility Delete', toShow: 
result }); 
    } 
    catch(err){ 
        res.status(500) 
